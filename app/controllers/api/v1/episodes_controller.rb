    ...
    class EpisodesController < Api::V1::ApiController
      ...
      def index
        render_success_response({
          episodes: array_serializer.new(tv_show.episodes, serializer: EpisodeSerializer)
        }, status = 200)
      end

      def show
        render_success_response({
          episode: single_serializer.new(@episode, serializer: EpisodeSerializer)
        })
      end
      ...
      def set_episode
        @episode = @tv_show.episodes.find(id: params[:id])
      end
    end
  end
end
